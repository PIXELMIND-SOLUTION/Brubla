                        ...
                        <div className="pt-4">
                            {filtered.length > 0 ? (
                                <>
                                    ...
                                    {filtered.map(s => (
                                        <button key={s.label} className="w-full flex items-center justify-between py-2.5 px-1 rounded-xl transition-colors hover:bg-[#fff]" onClick={onClose}>
                                            ...
                                            <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: "#fff", color: "#7a6a5a" }}>{s.tag}</span>
                                        </button>
                                    ))}
                                </>
                            ) : (
                                <div className="flex flex-col items-center justify-center py-10 gap-2">
                                    ...
                                    <p className="text-sm font-medium" style={{ color: "#000" }}>No results for "<span style={{ color: "#000" }}>{q}</span>"</p>
                                    <p className="text-xs" style={{ color: "#000" }}>Try searching for something else</p>
                                </div>
                            )}
                        </div>
                    )}
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {RECENT.map(r => (
                                            <button key={r} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-colors hover:bg-[#f0ebe4]"
                                                style={{ background: "#fff", color: "#333", border: "1px solid rgba(111,78,55,0.15)" }} onClick={onClose}>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {TRENDING.map((t, i) => (
                                        <button key={t} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all hover:scale-105"
                                            style={{ background: i === 0 ? "linear-gradient(135deg,#fff,#fff)" : "#fff", color: i === 0 ? "#000" : "#333", border: "1px solid rgba(111,78,55,0.15)" }}
                                            onClick={onClose}>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {SUGGESTIONS.slice(0, 4).map(s => (
                                        <button key={s.label} className="flex items-center justify-between px-3 py-2.5 rounded-xl transition-colors hover:bg-[#f0ebe4] text-left"
                                            style={{ background: "#fff", border: "1px solid rgba(111,78,55,0.12)" }} onClick={onClose}>
...
const CollectionsPanel = ({
    ...
    const fetchCollections = async () => {
        try {
            ...
            if (result.success) {
                const formattedCollections = result.data
                ...
                setCollectionItems(formattedCollections);
            }
        } catch (error) {
