...
const NotifBanner = ({ onClose }) => {
    ...
    return (
        <div className="h-10 flex items-center justify-between px-3 md:px-6 overflow-hidden" style={{ background: "#000" }}>
            ...
            <div className="flex items-center gap-2 md:gap-3 ml-2 md:ml-3">
                ...
                <button className="flex items-center gap-1 md:gap-2 text-[11px] md:text-xs font-medium hover:opacity-80 text-white">
                    <FaQuestionCircle className="text-sm" />
                    <span className="hidden md:inline">Help</span>
                </button>
                <button onClick={onClose} className="opacity-40 hover:opacity-80 transition text-white">
    ...
    return (
        <div className="relative" ref={ref}>
            <button onClick={() => setOpen(o => !o)}
                ...
                style={{ color: "#333" }} aria-label="Change location">
                <PinIcon c="w-5 h-5" />
...
const CollectionsPanel = ({ show, onMouseEnter, onMouseLeave, navbarHeight }) => {
    ...
    return (
        <div onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}
            ...
            style={{
                top: `${navbarHeight}px`, background: "#f9f5f0",
                borderBottom: "1px solid rgba(111,78,55,0.15)",
            ...
            <div className="px-4 md:px-8 lg:px-12 py-6">
                ...
                <div className="hide-sb flex gap-4 md:gap-5 overflow-x-auto pb-2" style={{ scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
                    {items.map((item, i) => (
                        <div key={i} onClick={() => navigate(`/collections/${item.id}`)}
...
const MegaMenuPanel = ({ show, onClose, navbarHeight }) => {
    ...
    const ColorDot = ({ color }) => {
        ...
        return (
            <button
                ...
                title={color}
            >
                <span className="w-7 h-7 rounded-full border-2 border-white shadow transition-transform group-hover:scale-110"
    ...
    return (
        <div className="fixed left-0 w-full z-40 transition-all duration-300"
            style={{
                top: `${navbarHeight}px`, background: "#f9f5f0",
                borderBottom: "1px solid rgba(111,78,55,0.15)",
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 md:gap-8">
                        ...
                        <div>
                            ...
                            <div className="space-y-1">
                                {categories.map((cat, i) => (
                                    <button key={cat.id} onClick={() => handleCatClick(i)}
                            ...
                            {activeCat?.subcategories?.length ? (
                                <div className="space-y-1">
                                    {activeCat.subcategories.map((sub, i) => (
                                        <button key={sub.id} onClick={() => { setActiveSubIdx(i); navigate(`/products?subcategory=${sub.id}`); }}
                        ...
                        <div>
                            ...
                            {sizes.length ? (
                                ...
                                <p className="text-xs text-[#7a6a5a] px-1">No sizes available</p>
                            )}
        ...
        <>
            ...
            <aside className="fixed top-0 left-0 h-full z-[400] flex flex-col transition-transform duration-300 ease-out"
                            ...
                            <button key={id} onClick={() => handleNav(id, link)}
                                ...
                                {special && !isActive && (
                                    <span className="ml-auto text-[9px] font-black px-2 py-0.5 rounded-full"
                                        style={{ background: "linear-gradient(135deg,#000,#8B6347)", color: "#fff" }}>NEW</span>
                                )}
                ...
                <div className="px-5 py-4" style={{ borderTop: "1px solid rgba(111,78,55,0.12)" }}>
                    <div onClick={() => navigate("/profile")} className="flex items-center gap-3 p-3 rounded-xl cursor-pointer" style={{ background: "#f9f5f0" }}>
...
const CartBtn = ({ count, transparent }) => (
    <Link to="/mycart" className="relative p-2 rounded-full transition-colors hover:bg-white/10" aria-label="View cart"
...
const BrublaWordmark = ({ transparent, onClick }) => (
    <button onClick={onClick} className="flex flex-col items-center leading-none select-none cursor-pointer">
        <span className="font-black uppercase tracking-[0.25em] transition-colors duration-300"
...
const Navbar = () => {
    ...
    useEffect(() => {
        ...
        fetchCart();
        const id = setInterval(fetchCart, 5000); // reduced from 2s to 5s
    ...
    const TAB_PILLS = [
        ...
        { label: "Collections", onMouseEnter: () => { setShowCollections(true); setOpenMenu(false); } },
        ...
        { label: "Sale", onClick: () => navigate("/products?sale=true"), red: true },
    ];
            ...
            <header ref={headerRef} className="fixed top-0 left-0 right-0 z-[500] transition-all duration-300">
                ...
                <div className="transition-all duration-300"
                    style={{
                        ...
                        boxShadow: transparent ? "none" : scrolled ? "0 4px 24px rgba(0,0,0,0.09)" : "0 1px 4px rgba(0,0,0,0.04)",
                    }}>
                    ...
                    <div className="hidden lg:grid items-center h-16 px-8" style={{ gridTemplateColumns: "1fr auto 1fr" }}>
                        ...
                        <div className="flex items-center gap-2">
                            <div onClick={() => navigate("/home")} className="cursor-pointer flex-shrink-0">
                                <img src="/logo2.png" className={`h-10 w-10 ${transparent ? "bg-white" : ""}`} alt="logo"
                        ...
                        {/* CENTER */}
                        <BrublaWordmark transparent={transparent} onClick={() => navigate("/home")} />

                        {/* RIGHT */}
                        <div className="flex items-center justify-end gap-0.5">
                            <button onClick={openSearch} className="p-2 rounded-full transition-colors hover:bg-white/10" style={{ color: iconColor }}>
                                <SearchIcon c="w-5 h-5" />
                            </button>
                            <CartBtn count={cartCount} transparent={transparent} />
                            <LocationSelector />
                            <button onClick={() => navigate("/profile")} className="p-2 rounded-full transition-colors hover:bg-white/10" style={{ color: iconColor }}>
                    ...
                    <div className="lg:hidden flex items-center gap-2 px-3 md:px-5 h-14">
                        <button onClick={openSidebar} className="p-2 rounded-full transition-colors hover:bg-white/10 flex-shrink-0" style={{ color: iconColor }}>
                            <MenuIcon c="w-5 h-5" />
                        </button>
                        <div onClick={() => navigate("/home")} className="cursor-pointer flex-shrink-0">
                            <img src="/logo2.png" className="h-10 w-10" alt="logo" onError={e => { e.target.style.display = "none"; }} />
